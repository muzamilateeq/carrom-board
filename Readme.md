...
## 📁 Clean Directory & Modular Folder Structure
...
| :--- | :--- | :--- |
| **Preloader** | `src/loading/` | Asynchronously preloads all image assets before unveiling the main menu. |
| **Main Menu** | `src/menu/` | Player name editing, local storage initialization, free coins claim (`+5000`), and mode selection. |
| **Board Selection** | `src/board-selection/` | Multi-tier board selection cards with dynamic fee deduction and prize pool displays. |
| **Matchmaking** | `src/searching-player/` | Simulated matchmaking radar, opponent pairing, countdown timer, and game launch. |
| **Physics Engine** | `src/board-and-pieces/` & `game.js` | Matter.js 2D rigid-body simulation for striker aiming, sliding friction, pocket collisions, and turn switching. |
| **Match Result** | `src/result/` | Victory/Loss modal, prize distribution, and instant same-opponent rematch system. |
...
### Physics Parameters (`src/board-and-pieces/pieces-config.js`)
